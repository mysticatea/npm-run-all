...
describe("[argument-placeholders]", () => {
    ...
    describe("If arguments preceded by '--' are nothing, '{1}' should be empty:", () => {
        it("Node API", () =>
            nodeApi("test-task:dump {1}").then(() => assert(result() === "[]")))

        it("npm-run-all command", () =>
            runAll(["test-task:dump {1}"]).then(() =>
                assert(result() === "[]")
            ))
    ...
    describe("'{1}' should be replaced by the 1st argument preceded by '--':", () => {
        ...
        it("npm-run-all command", () =>
            runAll(["test-task:dump {1}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st"]')
            ))

        it("run-s command", () =>
            runSeq(["test-task:dump {1}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st"]')
            ))

        it("run-p command", () =>
            runPar(["test-task:dump {1}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st"]')
            ))
    })
    ...
    describe("'{2}' should be replaced by the 2nd argument preceded by '--':", () => {
        it("Node API", () =>
            nodeApi("test-task:dump {2}", {
                arguments: ["1st", "2nd"],
            }).then(() => assert(result() === '["2nd"]')))

        it("npm-run-all command", () =>
            runAll(["test-task:dump {2}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["2nd"]')
            ))

        it("run-s command", () =>
            runSeq(["test-task:dump {2}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["2nd"]')
            ))

        it("run-p command", () =>
            runPar(["test-task:dump {2}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["2nd"]')
            ))
    })
    ...
    describe("'{@}' should be replaced by the every argument preceded by '--':", () => {
        it("Node API", () =>
            nodeApi("test-task:dump {@}", {
                arguments: ["1st", "2nd"],
            }).then(() => assert(result() === '["1st","2nd"]')))

        it("npm-run-all command", () =>
            runAll(["test-task:dump {@}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st","2nd"]')
            ))

        it("run-s command", () =>
            runSeq(["test-task:dump {@}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st","2nd"]')
            ))

        it("run-p command", () =>
            runPar(["test-task:dump {@}", "--", "1st", "2nd"]).then(() =>
                assert(result() === '["1st","2nd"]')
            ))
    })
    ...
    describe("Every '{1}', '{2}', '{@}' and '{*}' should be replaced by the arguments preceded by '--':", () => {
        it("Node API", () =>
            nodeApi("test-task:dump {1} {2} {3} {@} {*}", {
                arguments: ["1st", "2nd"],
            }).then(() =>
                assert(result() === '["1st","2nd","1st","2nd","1st 2nd"]')
            ))

        it("npm-run-all command", () =>
            runAll([
                "test-task:dump {1} {2} {3} {@} {*}",
                "--",
                "1st",
                "2nd",
            ]).then(() =>
    ...
    describe("'{1:-foo}' should be replaced by 'foo' if arguments are nothing:", () => {
        it("Node API", () =>
            nodeApi("test-task:dump {1:-foo} {1}").then(() =>
                assert(result() === '["foo"]')
            ))

        it("npm-run-all command", () =>
            runAll(["test-task:dump {1:-foo} {1}"]).then(() =>
                assert(result() === '["foo"]')
            ))

        it("run-s command", () =>
            runSeq(["test-task:dump {1:-foo} {1}"]).then(() =>
                assert(result() === '["foo"]')
            ))

        it("run-p command", () =>
            runPar(["test-task:dump {1:-foo} {1}"]).then(() =>
                assert(result() === '["foo"]')
            ))
    })
