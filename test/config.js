...
describe("[config] it should have an ability to set config variables:", () => {
    ...
    describe("CLI commands should address \"--a=b\" style options", () => {
        it("npm-run-all command", async () => {
            await util.runAll(["test-task:config", "--test=GO"])
            assert(result() === "GO")
        })

        it("run-s command", async () => {
            await util.runSeq(["test-task:config", "--test=GO"])
            assert(result() === "GO")
        })

        it("run-p command", async () => {
            await util.runPar(["test-task:config", "--test=GO"])
            assert(result() === "GO")
        })
    })
    ...
    describe("CLI commands should address \"--b=c\" style options for multiple variables", () => {
        it("npm-run-all command", async () => {
            await util.runAll(["test-task:config2", "--test=1", "--test2=2", "--test3=3"])
            assert(result() === "1\n2\n3")
        })

        it("run-s command", async () => {
            await util.runSeq(["test-task:config2", "--test=1", "--test2=2", "--test3=3"])
            assert(result() === "1\n2\n3")
        })

        it("run-p command", async () => {
            await util.runPar(["test-task:config2", "--test=1", "--test2=2", "--test3=3"])
            assert(result() === "1\n2\n3")
        })
    })
    ...
    describe("CLI commands should transfar configs to nested commands.", () => {
        it("npm-run-all command", async () => {
            await util.runAll(["test-task:nested-config", "--test=GO DEEP"])
            assert(result() === "GO DEEP")
        })

        it("run-s command", async () => {
            await util.runSeq(["test-task:nested-config", "--test=GO DEEP"])
            assert(result() === "GO DEEP")
        })

        it("run-p command", async () => {
            await util.runPar(["test-task:nested-config", "--test=GO DEEP"])
            assert(result() === "GO DEEP")
        })
    })
})
